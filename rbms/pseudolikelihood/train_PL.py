...
def train_PL2(
    ...
    optimizer = Adam(params.parameters(), lr=learning_rate, weight_decay=0.005)#, maximize=True)
    ...
    for idx in range(num_updates + 1, args["num_updates"] + 1):
        ...
        loss = params.compute_loss_PL2(batch[0], l=args["lambda"], use_fields=use_fields)
        if args["verbose"]==True:
            print("Update: ", idx, "Loss:", loss.item(), "lr:", lr_scheduler.get_last_lr(), "J_norm:", torch.norm(params.K2).item(), "h_norm:", torch.norm(params.vbias).item())
